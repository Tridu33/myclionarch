...
//==============================for LeetCode
DoubleLink<int>* arr2Dlinklist(vector<int> & arr){
    // 创建双向链表
    DoubleLink<int>* pdlink = new DoubleLink<int>();
    for(int i =0;i<arr.size();i++){
        pdlink->append_last(arr[i]);    // 将 arr 中第i个元素追加到链表末尾
    }
    return pdlink;
}
void reversePrintDLinkList(DoubleLink<int>* pdlink){
    // 打印双向链表中的全部数据
    int sz = pdlink->size();
    for (int i=sz-1; i>=0; i--)
    {
        cout <<  pdlink->get(i) <<endl;
    }
    return ;
}
int main()
{
    ...
    DoubleLink<int>* head = arr2Dlinklist(input);
    reversePrintDLinkList(head);
    return 0;
}